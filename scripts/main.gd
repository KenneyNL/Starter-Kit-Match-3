...
func _ready():
	...
	get_viewport().size_changed.connect(reposition_board)

func center_grid_on_screen():
	
	container.position = (get_viewport_rect().size / 2.0) - (Vector2(width, height) * offset / 2.0)
	container.position += Vector2(offset / 2.0, offset / 2.0)

func reposition_board():
	
	center_grid_on_screen()
	for x in width:
		for y in height:
			var piece = grid[x][y]
			if piece: piece.position = grid_to_pixel(x, y)
...
func spawn_at(x, y):
	...
	created_piece.grid_pos = Vector2i(x, y) 
	...
	for y in height:
		for x in range(width - 2):
			...
			if p1 and p2 and p3 and p1.type == p2.type and p1.type == p3.type:
				for p in [p1, p2, p3]:
					if not p in matched_pieces: matched_pieces.append(p)
...
func process_board_state():
	...
	while matches.size() > 0:
		...
		for piece in matches:
			...
			grid[piece.grid_pos.x][piece.grid_pos.y] = null
